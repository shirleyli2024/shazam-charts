...
def truncate_data(conn):
    ...
    cursor = conn.cursor()
    cursor.execute('''DROP TABLE IF EXISTS ShazamGlobalGenre''')
    conn.commit()
...
def fetch_and_store_genre_data(genreName, conn):
    try:
        ...
        if cleaned_data:
            ...
            df.to_sql('ShazamGlobalGenre', conn, if_exists= 'append', index=False)
            print(f"Data for genre {genreName} successfully loaded into the database.")
        ...
        print(f"Request error for genre {genreName}: {e}")
    except Exception as e:
        print(f"An error occurred while processing genre {genreName}: {e}")
